...
use crate::types::rate_limits::RateLimitInterval;
use crate::types::rate_limits::RateLimitType;

#[derive(Debug)]
pub struct BinanceResponseWithMeta<T> {
    pub meta: BinanceResponseMeta,
    pub payload: T,
}

#[derive(Debug)]
pub struct BinanceResponseMeta {
    pub http_status: StatusCode,
    pub id: Option<Uuid>,
    pub usage: Vec<(RateLimitType, RateLimitInterval, u32, u64)>,
}

impl<T> BinanceResponseWithMeta<T> {
    pub fn new(meta: BinanceResponseMeta, payload: T) -> Self {
        BinanceResponseWithMeta { meta, payload }
    }

    pub fn into_parts(self) -> (BinanceResponseMeta, T) {
        (self.meta, self.payload)
    }

    pub fn into_meta(self) -> BinanceResponseMeta {
        self.meta
    }
            ...
            if let Some(value) = v.to_str().ok() {
                for (typ, prefix) in [
                    ...
                    if let Some((interval, quantity, used)) = parse_usage(prefix, k, value) {
                        tracing::debug!("    ::  {typ:?} {quantity} {interval:?} {used}");
