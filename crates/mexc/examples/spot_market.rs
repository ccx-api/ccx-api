...
async fn main_() -> MexcResult<()> {
    ...
    print_res(mexc_spot.depth(SYMBOL, OrderBookLimit::N10)?.await)?;
    // print_res(mexc_spot.depth(SYMBOL, None).await)?;
    // print_res(mexc_spot.trades(SYMBOL, None).await)?;
    // print_res(mexc_spot.historical_trades(SYMBOL, None, None).await)?;
    // print_res(mexc_spot.agg_trades(SYMBOL, None, None, None, None).await)?;
    // print_res(mexc_spot.klines(SYMBOL, ChartInterval::Minute1, None, None, None).await)?;
