...
pub const BINANCEPAY_OPENAPI_SUBMERCHANT_MODIFY: &str = "/binancepay/openapi/submerchant/modify";

#[derive(Debug, Serialize, Deserialize, Clone)]
#[serde(rename_all = "camelCase")]
pub struct V1SubmerchantModifyRequest {
    /// maximum length 128
    /// Sub-merchant ID returned by create sub-merchant api should be all number.
    ...
    /// maximum length 500
    /// Mandatory if `sub_pay_mcc_code` is 9999.
    /// Please specify the industry of this sub merchant here.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct V1SubmerchantModifyResult;

impl<S: crate::client::BinancePaySigner> Api<S> {
    pub async fn v1_submerchant_modify(
        &self,
        request: V1SubmerchantModifyRequest,
        time_window: impl Into<Time>,
    ) -> LibResult<BinancePayResponse<V1SubmerchantModifyResult>> {
        self.client
            .post_json(BINANCEPAY_OPENAPI_SUBMERCHANT_MODIFY, request)?
            .signed(time_window)?
            .random_nonce()?
            .send()
            .await
...
mod tests {
    ...
    #[test]
    fn v1_test_serde_modify_submerchant_individual_request() {
        let json = r#"
        {
    ...
    #[test]
    fn v1_test_serde_modify_submerchant_response() {
        let example = r#"
        {
            "status": "SUCCESS",
            "code": "000000"
